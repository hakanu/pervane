...
class MyView(BaseView):
  ...
  @expose('/api/check_updates')
  @login_required
  def api_check_updates(self):
  ...
  @expose('/api/get_content')
  @login_required
  def api_get_content_handler(self):
    requested_path = _get_request_param('f')
    path, err = _get_real_path(requested_path)
  ...
  @expose('/api/update', methods=['POST'])
  @login_required
  def api_update_handler(self):
  ...
  @expose('/api/add_node', methods=['POST'])
  @login_required
  ...
  @expose('/api/move_file')
  @login_required
  def api_move_handler(self):
  ...
  @expose('/_img/<path:file_path>', methods=['GET'])
  @login_required
  def static_file_handler(self, file_path):
...
logging.info('config dir: %s', _PERVANE_CONFIG_DIR)
...
logging.info('db path: %s', _SQLITE_PATH)
ALLOWED_EXTENSIONS = {'txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif'}
...
def main():
  if args.mode == 'serve':
    ...
    import os
    os.environ['FLAKS_APP'] = 'run_create_admin'
